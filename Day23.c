...
static Amps ampsFromMap(Map map) {
    Amps amps = {0};
    int n = 0;

    for (int y = 0; y < 5; ++y) {
        for (int x = 0; x < 13; ++x) {
            if (map.m[y][x] >= 'A' && map.m[y][x] <= 'D') {
                // amps.as[n].t = map.m[y][x];
                amps.as[n].x = x;
                amps.as[n].y = y;
                ++n;
            }
        ...
        if (empty) {
            ...
            if (empty) {
                ...
                if (map.m[3][destX] == 0) {
                    ...
                    maps.ms[maps.n].amp[0] = destX;
                    maps.ms[maps.n].amp[1] = 2;
            ...
            for (int x2 = x - 1; x2 >= 0; --x2) {
                ...
                maps.ms[maps.n].amp[0] = x2;
                maps.ms[maps.n].amp[1] = 1;
...
static int64_t partOne(Map map) {
    // Move middle B
    map.amp[0] = 7;
    map.amp[1] = 2;
